...
#ifdef STC_GTYPE_BRANCHDETAIL
#define STC_GTYPE_BRANCH
#endif
...
void OffsetSTCShaderVertex(inout STCShaderVB data, float lodValue) {
CustomWind();
...
#endif
#if defined(STC_GTYPE_BRANCH) || defined(STC_GTYPE_FROND)
#ifdef LOD_FADE_PERCENTAGE
finalPosition = lerp(finalPosition, data.texcoord1.xyz, lodValue);
#endif
#if defined(ENABLE_WIND) && defined(STC_GTYPE_FROND)
...
#if defined(STC_GTYPE_FROND) || defined(STC_GTYPE_LEAF) // defined(GEOM_TYPE_FACING_LEAF)
#define STCShader_ALPHATEST
fixed _Cutoff;
#endif
...
sampler2D _SpecMap;
half _Gloss, _Shininess, _SpecMapInts;
#endif
...
#ifdef COLOR_TUNING
half _Light, _Saturation, _Contrast, _Brightness;
#endif
...
void STCShaderVert(inout STCShaderVB IN, out Input OUT) {
...
#endif
OffsetSTCShaderVertex(IN, unity_LODFade.x);
}
...
#ifdef EFFECT_SPECULAR
#define STCShader_DATA_GLOSS half3 Gloss;
#define STCShader_COPY_GLOSS(to, from) to.Gloss = from.Gloss;
#define STCShader_DATA_SPECULAR half3 Specular;
#define STCShader_COPY_SPECULAR(to, from) to.Specular = from.Specular;
#else
#define STCShader_DATA_GLOSS
#define STCShader_COPY_GLOSS(to, from)
#define STCShader_DATA_SPECULAR
#define STCShader_COPY_SPECULAR(to, from)
#endif

#ifdef EFFECT_EMISSION
#define STCShader_DATA_EMISSION half3 Emission;
#define STCShader_COPY_EMISSION(to, from) to.Emission = from.Emission;
#else
#define STCShader_DATA_EMISSION
#define STCShader_COPY_EMISSION(to, from)
#endif

#if defined(EFFECT_BUMP) && !defined(LIGHTMAP_ON)
#define STCShader_DATA_NORMAL half3 Normal;
#define STCShader_COPY_NORMAL(to, from) to.Normal = from.Normal;
#else
#define STCShader_DATA_NORMAL
#define STCShader_COPY_NORMAL(to, from)
#endif

#ifdef STCShader_ALPHATEST
#define STCShader_DATA_ALPHA half Alpha;
#define STCShader_COPY_ALPHA(to, from) to.Alpha = from.Alpha;
#else
#define STCShader_DATA_ALPHA
#define STCShader_COPY_ALPHA(to, from)
#endif

#define STCShader_COPY_FRAG(to, from) \
to.Albedo = from.Albedo; \
STCShader_COPY_ALPHA(to, from) \
STCShader_COPY_GLOSS(to, from) \
STCShader_COPY_SPECULAR(to, from) \
STCShader_COPY_EMISSION(to, from) \
STCShader_COPY_NORMAL(to, from)

struct STCShaderFragOut {
half3 Albedo;
STCShader_DATA_ALPHA
STCShader_DATA_GLOSS
STCShader_DATA_SPECULAR
STCShader_DATA_EMISSION
STCShader_DATA_NORMAL
};
...
void STCShaderFrag(Input IN, out STCShaderFragOut OUT) {
...
#endif
#else
fixed4 diffuseColor = fixed4(1,1,1,1);
#ifdef STC_GTYPE_BRANCHDETAIL
fixed4 detailColor = diffuseColor;
#endif
#endif
...
#ifdef EFFECT_SPECULAR
OUT.Gloss = _Gloss;
fixed4 specTex = tex2D(_SpecMap, IN.mainTexUV);
half specTexC = ((specTex - 0.5) * _SpecMapInts + 0.5).g;
OUT.Specular = _Shininess * specTexC;
#endif

// emissive
#ifdef EFFECT_EMISSION
OUT.Emission = tex2D(_EmissionTex, IN.mainTexUV) *_EmissionColor.rgb;
#endif
